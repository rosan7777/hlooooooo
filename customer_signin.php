...
if (isset($_POST["sign_in"])) {
    ...
    if (empty($email) || empty($password) || empty($user_role)) {
        ...
        if ($user_role === 'customer') {
            ...
            WHERE 
                HU.USER_EMAIL = :email AND HU.USER_TYPE = 'customer'";
            ...
            WHERE 
                HU.USER_EMAIL = :email AND HU.USER_TYPE = 'trader'";
            ...
            FROM 
                CLECK_USER
            WHERE 
                USER_EMAIL = :email AND USER_TYPE = 'admin'";
        }
        ...
        if ($row = oci_fetch_assoc($stmt)) {
            if (password_verify($password, $row["USER_PASSWORD"])) {
                if ($user_role === 'customer' && $row["VERIFIED_CUSTOMER"] != 1) {
                    ...
                    // Handle "Remember me" with a secure token
                    if (isset($_POST["remember"])) {
                        $token = bin2hex(random_bytes(32)); // Generate a secure token
                        setcookie("remember_token", $token, time() + (86400 * 30), "/"); // 30 days
                        // TODO: Store token in database (e.g., UPDATE CLECK_USER SET remember_token = :token WHERE USER_ID = :user_id)
                    } else {
